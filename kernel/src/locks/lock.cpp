...
uint64_t AtomicCompareExchange(uint64_t *current, uint64_t currentValue, uint64_t newValue)
{
    volatile uint64_t *volatileCurrentPointer = (volatile uint64_t *)(current);
    ...
    asm volatile("lock; cmpxchgq %2,%1"
                 : "=a"(operationResult), "+m"(*volatileCurrentPointer)
